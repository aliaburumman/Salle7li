...
const OrderSummary = ({route}: any) => {
  ...
  const totalCost =
    calculateTotal(values.availabilityTime) * (1 - percentage / 100);
    const [amount,setAmount]=useState(0);
  useEffect(() => {setAmount(totalCost)}, [totalCost, applyButtonText]);
  ...
  const handleCheckPromoCode = async () => {
    try {
      console.log('here');
      const response = await checkPromoCode({promoCode: promoCode}).unwrap();
      console.log(response, 'erere');
